...
def calcul_score(case) :
    """ renvoie le nombre de points à ajouter au score
    case vide => -1
    pac_gomme => +1
    """
    if case == 0 :
...
# 2 : vers le bas
    ...
    if touche == '8' : # mouvement vers le haut
        if plateau[lin-1][col] != -1 :
            ...
            plateau [lin][col] = 0
            lin = lin - 1
            score = score + calcul_score(plateau[lin][col])  # on met à jour le score
            plateau [lin][col] = 7
